...
Crusty is a new programming language that provides a C-like syntax while guaranteeing Rust compatibility. Phase 1 focuses on building two tools: `crustyc` (a bidirectional compiler-compiler/transpiler that translates between Crusty and Rust source code) and `crustydoc` (a documentation generator that leverages the Linux kernel document object model for self-documented code). This phase serves as an experimental platform to determine which C language features can be preserved and which must be excluded from the Crusty dialect to maintain Rust compatibility.

## Glossary

- **Crusty**: The new C-like programming language being developed
- **crustyc**: The compiler binary that translates between Crusty and Rust source code
- **crustydoc**: The documentation generator binary that produces documentation from Crusty source code
- **Transpiler**: The compiler-compiler that translates source code between languages
...
- **Developer**: A programmer working on the Crusty compiler or writing Crusty programs
- **Crusty_Repository**: The official Crusty compiler repository at https://github.com/major0/crusty.git
...
**User Story:** As a compiler developer, I want a GitHub CI/CD pipeline that automatically builds and tests all code, so that I can ensure code quality and catch regressions early.
...
**User Story:** As a compiler developer, I want to commit code changes after each task completion using Conventional Commits format, so that I can track progress and maintain a clear, standardized development history.
...
### Requirement 6: Parse Crusty Source Code

**User Story:** As a compiler developer, I want to parse Crusty source files, so that I can validate syntax and build an internal representation.
...
### Requirement 7: Validate Semantic Correctness

**User Story:** As a compiler developer, I want to validate program semantics, so that only well-formed programs are transpiled.
...
### Requirement 8: Generate Rust Code

**User Story:** As a compiler developer, I want to generate valid Rust code from Crusty programs, so that the output can be compiled with rustc.
...
### Requirement 9: Compile Generated Rust Code

**User Story:** As a compiler user, I want the generated Rust code to compile successfully, so that I can execute my Crusty programs.
...
### Requirement 10: Report Compilation Errors
...
### Requirement 11: Handle File Input and Output

**User Story:** As a Crusty programmer, I want to compile source files from the command line using crustyc, so that I can integrate the compiler into my workflow.
...
### Requirement 12: Support Command-Line Options

**User Story:** As a Crusty programmer, I want to control compiler behavior through command-line options, so that I can customize the build process.

#### Acceptance Criteria

1. THE crustyc SHALL support -o or --output option to specify the output file path
2. THE crustyc SHALL support --emit option to control what gets generated (rust, binary, ast)
3. WHEN --emit=rust is specified, THE crustyc SHALL generate Rust source code only without invoking rustc
4. WHEN --emit=binary is specified or --emit is not provided, THE crustyc SHALL generate Rust code and compile it with rustc
5. WHEN --emit=ast is specified, THE crustyc SHALL output the parsed AST in a human-readable format
6. THE crustyc SHALL support -v or --verbose option for detailed compilation output
7. WHEN --verbose is specified, THE crustyc SHALL output detailed information about parsing, semantic analysis, and code generation stages
8. THE crustyc SHALL support --version option to display the compiler version
9. THE crustyc SHALL support -h or --help option to display usage information
10. THE crustyc SHALL support --no-compile option to generate Rust code without invoking rustc
11. THE crustyc SHALL support --from-lang option to specify the source language (crusty or rust)
12. WHEN multiple incompatible options are specified, THE crustyc SHALL report an error with explanation
13. THE crustyc SHALL display clear usage information when invoked with invalid options

### Requirement 13: Support Project Configuration with crusty.toml

**User Story:** As a Crusty programmer, I want to configure multi-file projects using crusty.toml, so that I can manage dependencies and build settings.

#### Acceptance Criteria

1. THE crustyc SHALL recognize crusty.toml files in the project root directory
2. THE crusty.toml file SHALL be 100% compatible with Cargo.toml syntax and structure
3. WHEN a crusty.toml file is present, THE crustyc SHALL parse it to determine project configuration
4. THE crustyc SHALL support [package] section with name, version, authors, and edition fields
5. THE crustyc SHALL support [dependencies] section for specifying Rust crate dependencies
6. THE crustyc SHALL support [dev-dependencies] section for test and development dependencies
7. THE crustyc SHALL support [[bin]] sections for defining binary targets
8. THE crustyc SHALL support [lib] section for defining library targets
9. WHEN building a project with crusty.toml, THE crustyc SHALL resolve all source files in the src directory
10. WHEN building a project with crusty.toml, THE crustyc SHALL pass dependency information to rustc or cargo
11. WHEN crusty.toml is malformed, THE crustyc SHALL report clear parsing errors
12. THE crustyc SHALL support building projects without crusty.toml for single-file programs
13. WHEN both crusty.toml and a source file are specified, THE crustyc SHALL use crusty.toml configuration
14. THE crustyc SHALL generate appropriate Cargo.toml when transpiling to Rust projects

### Requirement 14: Support Multi-File Projects
...
2. WHEN a crusty.toml file is present, THE crustyc SHALL discover all .crst source files in the src directory
...
7. WHEN generating Rust code for multi-file projects, THE Code_Generator SHALL create corresponding .rs files
...
### Requirement 15: Validate Program Entry Point
...
### Requirement 16: Provide Round-Trip Validation

**User Story:** As a compiler developer, I want to validate the transpilation process, so that I can ensure correctness.
...
### Requirement 17: Document Unsupported C Features
...
7. THE Transpiler SHALL document that #define macros are supported for macro definitions and translate to Rust macro_rules!

## Type System

### Requirement 18: Support Core C-like Type System
...
### Requirement 19: Support Tuple Types
...
### Requirement 20: Support Array and Collection Literals
...
### Requirement 21: Support Struct Methods and Implementation Blocks
...
### Requirement 22: Support Traits as C-Style VTable Structs
...
### Requirement 23: Support Using Rust Macros

**User Story:** As a Crusty programmer, I want to use Rust's standard macros with the same ! suffix syntax, so that I can leverage existing Rust functionality for common tasks with familiar syntax.

#### Acceptance Criteria

1. THE Parser SHALL support Crusty macro invocation syntax with ! suffix (macro_name!(args), macro_name![args], macro_name!{args})
2. THE Parser SHALL support common Rust macros with ! suffix (println!(...), print!(...), format!(...), vec![...], assert!(...), panic!(...), etc.)
3. THE Parser SHALL support macro invocations in expression contexts
4. THE Parser SHALL support macro invocations in statement contexts
5. THE Parser SHALL use ! suffix exclusively for macros, while @ prefix is exclusively for type-scoped calls
6. WHEN generating Rust code, THE Code_Generator SHALL pass through ! suffix macro invocations unchanged to Rust
7. THE Semantic_Analyzer SHALL validate macro arguments where possible
8. WHEN reverse transpiling from Rust, THE Code_Generator SHALL preserve Rust ! suffix macro syntax in Crusty unchanged

### Requirement 24: Support Defining Macros with #define

**User Story:** As a Crusty programmer, I want to define macros using #define syntax, so that I can create reusable code patterns that translate to Rust macro_rules!

#### Acceptance Criteria

1. THE Parser SHALL support #define directive for macro definitions
2. THE Parser SHALL support #define with macro name and parameter list (#define MACRO_NAME(param1, param2) body)
3. THE Parser SHALL support #define with macro name only for simple macros (#define MACRO_NAME body)
4. THE Parser SHALL support macro bodies containing expressions, statements, and code blocks
5. THE Parser SHALL support macro parameters in the macro body
6. THE Parser SHALL parse the macro body as a token sequence without full semantic analysis
7. WHEN generating Rust code, THE Code_Generator SHALL translate #define macros to Rust macro_rules! definitions
8. WHEN generating Rust code, THE Code_Generator SHALL translate macro parameters to Rust macro pattern variables ($param:expr)
9. WHEN generating Rust code, THE Code_Generator SHALL wrap the macro body in appropriate Rust macro syntax
10. THE Semantic_Analyzer SHALL validate that #define macros have valid syntax
11. THE Semantic_Analyzer SHALL verify that macro parameters are used consistently in the macro body
12. WHEN reverse transpiling from Rust, THE Code_Generator SHALL translate Rust macro_rules! definitions to Crusty #define syntax where possible
13. WHEN a Rust macro_rules! is too complex to represent as #define, THE Transpiler SHALL preserve it as rust! { macro_rules! ... }

### Requirement 25: Support Attributes
...
### Requirement 26: Support Rust Slice and Range Syntax
...
### Requirement 27: Support Type Casting
...
### Requirement 28: Support Sizeof Operator
...
### Requirement 29: Support C-Style Operators
...
### Requirement 30: Support Pointer Arithmetic with Rust Constraints
...
### Requirement 31: Support Type Aliases with Typedef
...
### Requirement 32: Support C-Style Enum Declarations
...
### Requirement 33: Support String Types
...
### Requirement 34: Support NULL and Option Type Mapping
...
### Requirement 35: Support Variable Mutability
...
### Requirement 36: Support References, Borrowing, and Lifetimes
...
### Requirement 37: Use Rust Standard Library Directly

**User Story:** As a Crusty programmer, I want to use Rust's standard library directly, so that I can leverage existing Rust functionality without reimplementation.
...
3. WHEN Crusty code uses #use directives, THE Code_Generator SHALL translate them directly to Rust use statements
4. THE Parser SHALL support importing any Rust standard library module via #use (e.g., #use std.collections.HashMap)
5. THE Parser SHALL support importing Rust types, functions, and traits from std modules
6. WHEN generating code, THE Code_Generator SHALL preserve all Rust std module references unchanged
7. THE Semantic_Analyzer SHALL validate that imported Rust std types and functions are used correctly
8. THE crustyc documentation SHALL reference Rust's standard library documentation for available functionality
9. WHEN a Crusty programmer needs I/O functionality, THEY SHALL use std.io via #use std.io
10. WHEN a Crusty programmer needs collections, THEY SHALL use std.collections via #use std.collections
11. WHEN a Crusty programmer needs file operations, THEY SHALL use std.fs via #use std.fs
12. WHEN a Crusty programmer needs string operations, THEY SHALL use Rust's String and &str types directly
13. WHEN a Crusty programmer needs memory allocation, THEY SHALL use Rust's Box, Rc, Arc, Vec types via #use std.boxed, std.rc, std.sync, std.vec
14. THE crustyc SHALL NOT wrap, abstract, or modify Rust standard library functionality
15. WHEN reverse transpiling from Rust, THE Code_Generator SHALL preserve all std module imports as Crusty #use directives

### Requirement 38: Support Generic Types with Explicit Type Parameters
...
15. WHEN explicit generic parameters are specified, THE Parser SHALL use parentheses for the outermost level
16. WHEN nested generic parameters are specified, THE Parser SHALL alternate between parentheses and brackets for each nesting level
17. WHEN generating Rust code, THE Code_Generator SHALL translate parentheses/brackets syntax to Rust's angle bracket turbofish syntax (<>)
18. WHEN generating Rust code, THE Code_Generator SHALL translate @Type(T) to Type::<T>
19. WHEN generating Rust code, THE Code_Generator SHALL translate @Type(Inner[T]) to Type::<Inner<T>>
20. WHEN generating Rust code, THE Code_Generator SHALL translate @Type(T1, T2) to Type::<T1, T2>
21. WHEN generic parameters are omitted, THE Code_Generator SHALL omit the turbofish syntax and rely on Rust's type inference
22. THE Semantic_Analyzer SHALL verify that parentheses and brackets alternate correctly in nested generics
23. THE Semantic_Analyzer SHALL verify that the number of type parameters matches the generic type definition
24. THE Semantic_Analyzer SHALL reject type-scoped calls without @ prefix

**Reverse Transpilation:**

25. WHEN reverse transpiling from Rust, THE Code_Generator SHALL preserve angle bracket generic type syntax in Crusty
26. WHEN reverse transpiling from Rust, THE Code_Generator SHALL translate Rust turbofish syntax (Type::<T>) to Crusty parentheses syntax (@Type(T))
27. WHEN reverse transpiling from Rust, THE Code_Generator SHALL translate nested angle brackets to alternating parentheses and brackets
28. WHEN reverse transpiling from Rust, THE Code_Generator SHALL translate Rust Type::method() to Crusty @Type.method()
...
### Requirement 39: Support C-Style Struct Initialization
...
### Requirement 40: Support Type Inference with Auto Keyword
...
### Requirement 41: Support Extern C Declarations for FFI
...
### Requirement 42: Support Inline Assembly
...
1. THE Parser SHALL support asm! macro syntax with ! suffix
...
5. WHEN generating Rust code, THE Code_Generator SHALL pass asm! macro invocations through to Rust unchanged
6. THE Semantic_Analyzer SHALL require asm! to be used within unsafe blocks
7. THE Semantic_Analyzer SHALL verify that asm! syntax is valid
8. WHEN reverse transpiling from Rust, THE Code_Generator SHALL preserve Rust asm! macro syntax in Crusty unchanged

### Requirement 43: Support Embedding Raw Rust Code
...
1. THE Parser SHALL support rust! macro syntax with ! suffix for embedding raw Rust code
2. THE Parser SHALL support rust! with block syntax (rust! { ... })
3. THE Parser SHALL treat the contents of rust! blocks as opaque Rust code
4. THE Parser SHALL support rust! in expression contexts
5. THE Parser SHALL support rust! in statement contexts
6. THE Parser SHALL support rust! in type contexts
7. WHEN generating Rust code, THE Code_Generator SHALL extract the contents of rust! blocks and emit them directly as Rust code
8. WHEN generating Rust code, THE Code_Generator SHALL preserve the exact formatting and syntax of embedded Rust code
9. THE Semantic_Analyzer SHALL NOT validate the contents of rust! blocks (validation deferred to rustc)
10. THE Semantic_Analyzer SHALL verify that rust! blocks are syntactically closed (matching braces)
11. WHEN reverse transpiling from Rust, THE Code_Generator MAY wrap Rust-specific syntax in rust! blocks when no Crusty equivalent exists
12. THE Parser SHALL support nested braces within rust! blocks
13. THE crustyc documentation SHALL recommend using rust! as an escape hatch for unsupported features

## Control Flow and Pattern Matching

### Requirement 44: Support For Loop Variants
...
### Requirement 45: Support Pattern Matching with Switch Statements
...
### Requirement 46: Support Error Handling with Fallible Return Types
...
### Requirement 47: Support Module Imports and Conditional Compilation
...
### Requirement 48: Support Namespace Declarations for Module Organization
...
### Requirement 49: Support Symbol Visibility with Underscore Prefix Convention
...
### Requirement 50: Extract Documentation from Source Code
...
### Requirement 51: Generate Documentation Using Kernel Document Object Model
...
### Requirement 52: Validate Documentation Completeness
...
### Requirement 53: Support Reverse Transpilation from Rust to Crusty
...
### Requirement 54: Provide Comprehensive Test Coverage for All Syntax Features

**User Story:** As a compiler developer, I want comprehensive tests for all Crusty syntax features, so that I can ensure correctness and prevent regressions.
